...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var inviteFriends = UIButton(frame: CGRect(x: 30 + 2 * (UIScreen.mainScreen().bounds.width - 40)/3, y: 10, width: (UIScreen.mainScreen().bounds.width - 40)/3, height: 40))
    ...
    override func viewDidLoad() {
        ...
        placeImageView.image = UIImage(named: "skyline")
        ...
        thingsLabel.text = "Space needle, science center, aquariums, football, movie theaters, rocky beaches, community fairs, etc."
        ...
        inviteFriends.setTitle("INVITE", forState: .Normal)
        inviteFriends.setTitleColor(UIColor.whiteColor(), forState: .Normal)
        inviteFriends.backgroundColor = UIColor(red: 0.2, green: 0.6, blue: 1, alpha: 1)
        inviteFriends.layer.cornerRadius = 5
        inviteFriends.titleLabel?.adjustsFontSizeToFitWidth = true
